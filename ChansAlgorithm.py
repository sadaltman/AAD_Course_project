...
def performance_analysis(dataset_sizes, distribution_name="custom"):
    """
    Analyzes performance of Chan's Algorithm across different input sizes.
    ...
    print("\n" + "=" * 60)
    print("PERFORMANCE ANALYSIS - CHAN'S ALGORITHM")
    print("=" * 60)
    ...
    for n, test_points in dataset_sizes:
        ...
        nlogh_values.append(n * math.log2(hull_size) if hull_size > 1 else n)
    ...
    filename1 = f'media/graphs/chans_algorithm_{distribution_name}_time_vs_n.png'
    plt.savefig(filename1, dpi=300, bbox_inches='tight')
    print(f"\n✓ Graph 1 (Time vs n) saved to: {filename1}")
    plt.close()
    
    # Graph 2: Time vs O(n log h) - Theoretical Complexity
    plt.figure(figsize=(12, 6))
    plt.plot(nlogh_values, times, 'yo-', linewidth=2, markersize=8)
    plt.xlabel('O(n log h)', fontsize=12, fontweight='bold')
    plt.ylabel('Execution Time (ms)', fontsize=12, fontweight='bold')
    plt.title(f"Chan's Algorithm: Time vs O(n log h)\n{distribution_name.replace('_', ' ').title()} Distribution", 
              fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    filename2 = f'media/graphs/chans_algorithm_{distribution_name}_time_vs_nlogh.png'
    plt.savefig(filename2, dpi=300, bbox_inches='tight')
    print(f"✓ Graph 2 (Time vs O(n log h)) saved to: {filename2}")
    plt.close()
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("CHAN'S ALGORITHM - CONVEX HULL ALGORITHM")
    print("=" * 60)
    ...
    if hull:
        ...
        print("Chan's algorithm failed to find convex hull!")
    
    
    print("\n" + "=" * 60)
    
    # Uncomment below to run performance analysis
    # Using larger input sizes (1K-500K) to clearly show O(n log h) behavior
